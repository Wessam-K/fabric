 ...
 */
function getCSP() {
  const backendOrigin = 'http://localhost:9002';
...
function detectDebugger() {
  ...
  // Check command line args for --inspect, --debug
...
function harden(win, logFn) {
  ...
  // Debugger detection
  if (detectDebugger()) {
    if (logFn) logFn('SECURITY_DEBUG_ATTEMPT', 'Debug mode detected in production');
  }
