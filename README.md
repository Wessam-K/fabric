# WK-Hub v7.1 — نظام إدارة المصنع 🏭

نظام ERP متكامل لإدارة مصانع الملابس. يشمل إدارة الموديلات، الأقمشة، الاكسسوارات، حساب التكاليف، الفواتير، أوامر العمل، الموردين، الموارد البشرية، الرواتب، الحضور، المستخدمين، سجل المراجعة، مركز التقارير المتقدم مع الجداول المحورية.

## v7.1 — تحسينات وإصلاحات (أحدث)

| # | الإصلاح | الأولوية |
|---|---------|----------|
| 1 | استبدال `window.confirm()` بمكون ConfirmModal في تفاصيل أمر العمل و BOM | حرج |
| 2 | خاصية `editable` تشمل حالات draft + pending + in_progress | حرج |
| 3 | زر "بدء" المرحلة يتطلب وجود قطع فعلية في المرحلة | عالي |
| 4 | محاذاة حقل `quantity_integrity` — `.ok` + `.difference` | عالي |
| 5 | لوحة تكاليف تقديرية حية في محرر BOM (sticky sidebar) | عالي |
| 6 | إزالة دالة `handleStageQty` الميتة | متوسط |
| 7 | شريط جانبي يعرض تفصيل القطع (مكتمل/في التصنيع/مرفوض/لم يبدأ) | متوسط |
| 8 | فتح نموذج التقدم تلقائياً لأول مرحلة نشطة | متوسط |
| 9 | سجل الحركة قابل للطي (مغلق افتراضياً) | منخفض |

## إحصائيات المشروع

| البند | العدد |
|-------|-------|
| إجمالي الملفات | **76 ملف** |
| ملفات الباكند | 22 ملف — 4,683 سطر |
| ملفات الفرونتند | 51 ملف — 9,000 سطر |
| ملفات الجذر | 3 ملفات (electron.js, package.json, README.md) |
| إجمالي أسطر الكود | **~14,054 سطر** |
| جداول قاعدة البيانات | **35 جدول** |
| صفحات الواجهة | **26 صفحة** |
| مكونات قابلة لإعادة الاستخدام | **14 مكون** |
| نقاط API | **65+ نقطة** |
| أدوار المستخدمين | **6 أدوار** |
...
| **v6.0** | **نظام الصلاحيات الدقيقة — 3 جداول + 6 endpoints + واجهة مصفوفة الصلاحيات + بيانات تجريبية شاملة (15 موظف، 7 مستخدمين، 765 سجل حضور)** |
| **v7.0** | **لوحة WIP موحدة — تمرير القطع بين المراحل + سجل التحركات + حماية التجاوز + تتبع المستخدم + فحص سلامة الكميات + 37 اختبار API** |

---

**WK-Hub** v7.0 — نظام إدارة المصنع المتكامل | 38+ جدول | 70+ API
