...
export default function GlobalSearch() {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        className="flex items-center gap-2 w-full px-3 py-2 rounded-lg bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-400 text-xs transition-colors border border-gray-200 dark:border-white/10">
        <Search size={14} />
        <span className="flex-1 text-right">بحث...</span>
        <kbd className="text-[10px] bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-gray-400 px-1.5 py-0.5 rounded font-mono">Ctrl+K</kbd>
      </button>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh]" onClick={() => setOpen(false)}>
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-[#1a1a2e] rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        {/* Search input */}
        <div className="flex items-center gap-3 px-4 border-b border-gray-200 dark:border-white/10">
          ...
          {query && <button onClick={() => setQuery('')}><X size={16} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" /></button>}
          <button onClick={() => setOpen(false)} className="text-[10px] text-gray-400 border border-gray-200 dark:border-white/10 rounded px-1.5 py-0.5 font-mono">ESC</button>
        </div>
        ...
        <div className="max-h-[50vh] overflow-y-auto" dir="rtl">
          ...
          {!loading && Object.entries(results).map(([cat, items]) => {
            ...
            return (
              <div key={cat}>
                <div className="px-4 py-2 text-[10px] font-bold text-gray-400 flex items-center gap-1.5 bg-gray-50 dark:bg-white/5 sticky top-0">
                  <Icon size={12} /> {meta.label} <span className="mr-auto text-gray-300">{items.length}</span>
                </div>
                {items.map((item, i) => (
                  <button key={i} onClick={() => go(cat, item)}
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-white/5 transition-colors text-right">
                    <span className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${meta.color}`}><Icon size={15} /></span>
                    <div className="min-w-0 flex-1">
                      <div className="font-bold text-sm truncate text-[#1a1a2e] dark:text-white">{item.name || item.description || item.customer_name || item.code}</div>
