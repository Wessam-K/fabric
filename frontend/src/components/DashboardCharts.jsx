...
function PipelineChart({ data, tooltipStyle }) {
...
function QualityChart({ data, isDark }) {
...
function TopModelsChart({ data, tooltipStyle, isDark }) {
  ...
  if (!topModelsData.length) return null;
...
export default function DashboardCharts({ data }) {
  ...
  const pipelineData = data?.production_pipeline ? Object.values(data.production_pipeline).some(v => v > 0) : false;
  const topModelsData = (data?.top_models || []).length > 0;
  if (!pipelineData && !topModelsData) return null;

  return (
    <div className="space-y-4">
      <h3 className="text-sm font-bold text-gray-900 dark:text-white">تحليلات الإنتاج</h3>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <ErrorBoundary compact><PipelineChart data={data} tooltipStyle={tooltipStyle} /></ErrorBoundary>
        <ErrorBoundary compact><QualityChart data={data} isDark={isDark} /></ErrorBoundary>
        <ErrorBoundary compact><TopModelsChart data={data} tooltipStyle={tooltipStyle} isDark={isDark} /></ErrorBoundary>
      </div>
    </div>
  );
}
