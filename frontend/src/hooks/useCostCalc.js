import { useMemo } from 'react';

/**
 * Cost calculation hook.
 *
 * Formulas:
 *   main_fabric_cost  = SUM of each main fabric: meters × price × (1 + waste%/100)
 *   lining_cost        = SUM of each lining fabric: meters × price  (NO waste)
 *   accessories_cost   = SUM of each accessory: quantity × price
 *   total_cost         = main_fabric_cost + lining_cost + accessories_cost + masnaiya + masrouf
 *   cost_per_piece     = total_cost / grand_total_pieces  (if grand_total > 0)
 */
export default function useCostCalc({ fabrics = [], accessories = [], masnaiya = 0, masrouf = 0, grandTotalPieces = 0 }) {
  return useMemo(() => {
    const safeParse = (v) => parseFloat(v) || 0;

    let main_fabric_cost = 0;
    let lining_cost = 0;

    for (const f of fabrics) {
      const meters = safeParse(f.meters);
      const price = safeParse(f.price_per_meter);
      const waste = safeParse(f.waste_pct);
      const role = f.role || 'main';

      if (role === 'lining') {
        lining_cost += meters * price;
      } else {
        main_fabric_cost += meters * price * (1 + waste / 100);
      }
    }

    let accessories_cost = 0;
    for (const a of accessories) {
      accessories_cost += safeParse(a.quantity) * safeParse(a.price);
    }

    const m = safeParse(masnaiya);
    const r = safeParse(masrouf);
    const total_cost = main_fabric_cost + lining_cost + accessories_cost + m + r;
    const gtp = Math.max(0, safeParse(grandTotalPieces));
    const cost_per_piece = gtp > 0 ? total_cost / gtp : 0;

    return {
      main_fabric_cost,
      lining_cost,
      accessories_cost,
      masnaiya: m,
      masrouf: r,
      total_cost,
      grand_total_pieces: gtp,
      cost_per_piece,
    };
  }, [fabrics, accessories, masnaiya, masrouf, grandTotalPieces]);
}
