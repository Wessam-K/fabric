...
router.post('/', (req, res) => {
  try {
    const { code, name, phone, email, address, city, tax_number, credit_limit, notes } = req.body;
    ...
    const result = db.prepare(`
      INSERT INTO customers (code, name, phone, email, address, city, tax_number, credit_limit, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(customerCode, name.trim(), phone || null, email || null, address || null, city || null, tax_number || null, credit_limit || 0, notes || null);
...
router.patch('/:id', (req, res) => {
  try {
    ...
    const { name, phone, email, address, city, tax_number, credit_limit, notes } = req.body;
