...
router.delete('/:code', requirePermission('fabrics', 'delete'), (req, res) => {
  try {
    ...
    const openPO = db.prepare("SELECT COUNT(*) as c FROM purchase_order_items pi JOIN purchase_orders po ON po.id = pi.po_id WHERE pi.fabric_code=? AND po.status IN ('sent','partial')").get(req.params.code).c;
