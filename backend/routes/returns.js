...
router.get('/sales', requirePermission('returns', 'view'), (req, res) => {
  try {
    ...
    const rows = db.prepare(`SELECT sr.*, c.name as customer_name, i.invoice_number
      ...
      WHERE ${where} ORDER BY sr.created_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
      ...
      for (const item of items) {
        if (item.item_type === 'fabric' && item.item_code) {
          ...
          db.prepare(`INSERT INTO fabric_stock_movements (fabric_code, movement_type, qty_meters, reference_type, reference_id, notes, created_by) VALUES (?,?,?,?,?,?,?)`)
            .run(item.item_code, 'return', item.quantity || 0, 'sales_return', id, 'مرتجع مبيعات ' + sr.return_number, req.user?.id || null);
...
router.get('/purchases', requirePermission('returns', 'view'), (req, res) => {
  try {
    ...
    const rows = db.prepare(`SELECT pr.*, s.name as supplier_name, po.po_number
      ...
      WHERE ${where} ORDER BY pr.created_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
    res.json({ data: rows, total, page: parseInt(page), pages: Math.ceil(total / limit) });
  } catch (err) { console.error(err); res.status(500).json({ error: 'حدث خطأ داخلي' }); }
});
