...
router.get('/sales', requirePermission('returns', 'read'), (req, res) => {
  try {
...
router.post('/sales', requirePermission('returns', 'create'), (req, res) => {
  try {
    ...
    const result = db.prepare(`INSERT INTO sales_returns (return_number, customer_id, invoice_id, return_date, reason, notes, subtotal, tax_amount, total, status, created_by)
...
router.get('/sales/:id', requirePermission('returns', 'read'), (req, res) => {
  try {
...
router.patch('/sales/:id/approve', requirePermission('returns', 'update'), (req, res) => {
  try {
...
router.get('/purchases', requirePermission('returns', 'read'), (req, res) => {
  try {
    const { status, page = 1, limit = 25 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);
    let where = '1=1'; const params = [];
...
router.post('/purchases', requirePermission('returns', 'create'), (req, res) => {
  try {
    ...
    const result = db.prepare(`INSERT INTO purchase_returns (return_number, supplier_id, purchase_order_id, return_date, reason, notes, subtotal, total, status, created_by)
      VALUES (?,?,?,datetime('now','localtime'),?,?,?,?,'draft',?)`)
...
router.get('/purchases/:id', requirePermission('returns', 'read'), (req, res) => {
  try {
...
router.patch('/purchases/:id/approve', requirePermission('returns', 'update'), (req, res) => {
  try {
    const id = parseInt(req.params.id);
